...
$\omega$ : & $\synt{World} \partialfunc \sigma$ & function representing each world's initial stack and final store \\\\
$\pi$ : & $2^{\synt{Node}}$ & current authorized set of principals \\\\
$\rho$ : & $\synt{Node}$ & current execution location \\\\
$\mu$ : & $(\synt{Sexp} \times \synt{Sexp}) \partialfunc \synt{Sexp}$ & function which merges two divergent data structures, or fails \\\\
$\Downarrow_{\synt{Com}}$ : & $(\synt{Com} \times \sigma \times \omega \times \pi \times \rho \times \mu) \partialfunc (\sigma \times \omega)$  & Commands step to a new top-level store and new set of hypothetical worlds\\\\
...
\AxiomC{$\forall v \not\in \sigma_{\text{hyp}}. \not\exists s. \sigma_{\text{merge}}[v] = s$}
\ELabel{Commit}
