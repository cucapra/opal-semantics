...
\begin{prooftree}
\AxiomC{$\config{c, (\sigma[(n, v) \mapsto \varnothing], \Sigma), \omega, \pi, \rho, \eta[op \mapsto (n, v, s_h)], \mu[(n, v) \mapsto s_m]} \Downarrow \config{\sigma', \omega'}$}
\AxiomC{$n \in \pi$}
\RightLabel{$\textsc{Handle}$}
\BinaryInfC{$\config{\handlecom{n}{v}{op}{s_h}{s_m}{c}, (\sigma, \Sigma), \omega, \pi, \rho, \eta, \mu} \Downarrow \config{\sigma', \omega'}$}
\end{prooftree}
...
\RightLabel{$\textsc{Op}$}
\BinaryInfC{$\config{\opcom{op}, \Sigma, \omega, \pi, \rho, \eta, \mu} \Downarrow \config{\sigma[(n, v) \mapsto s], \omega}$}
\end{prooftree}
...
\TrinaryInfC{$\sigma_{merge} (\sigma_{curr}, \Sigma_{curr}, \Sigma_{orig}, vo, vh, vc)) = \sigma_{curr}[(n, vo) \mapsto s_o, (n, vh) \mapsto s_h, (n, vc) \mapsto s_c]$}
\end{prooftree}
...
\AxiomC{$\sigma' = \sigma[\forall (n, v, s) \in \sigma_{\texttt{hyp}} \land \config{\mu((n, v)), \sigma_{merge}(\sigma_{curr}, \Sigma_{orig}, \Sigma_{curr}, vo, vh, vc), \pi} \Downarrow s_m, (n, v) \mapsto s_m]$}
\RightLabel{$\textsc{Commit}$}
\BinaryInfC{$\config{\commitcom{u}, (\sigma_{curr}, \Sigma_{curr}), \omega, \pi, \rho, \eta, \mu} \Downarrow \config{\sigma', \omega}$}
\end{prooftree}
...
Argument: \textsc{EqProp} and \textsc{MemProp} are well-founded proof trees since the sexp step is idempotent, and the two props decrease on size of sexp, so the maximum depth of the proof tree is proportional to the maximum depth of the sexp values
