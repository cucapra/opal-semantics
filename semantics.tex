...
\begin{prooftree}
\AxiomC{$\config{c, (\sigma_\varnothing, \Sigma), \omega, \pi, \rho, \eta, \mu} \Downarrow \config{\sigma, \omega'}$}
\RightLabel{$\textsc{Hyp}$}
\UnaryInfC{$\config{\hypcom{u}{c}, \Sigma, \omega, \pi, \rho, \eta, \mu} \Downarrow \config{\sigma, \omega[u \mapsto (\Sigma, \sigma)]}$}
\end{prooftree}
